...
export default function LoginPage() {
  ...
  const handleAuthenticationSubmit = async (e) => {
    ...
    try {
      ...
      setErrorStatusMessage(
        err.response?.data?.message || t.tryAgain,
      );
    } finally {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100 overflow-hidden px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="relative w-full max-w-5xl grid lg:grid-cols-2 bg-white rounded-[2rem] border border-slate-100 shadow-2xl shadow-slate-300/40 overflow-hidden animate-fade-in">
        ...
        <aside className="hidden lg:flex flex-col justify-between bg-gradient-to-br from-[#0B1528] via-[#112952] to-[#0A172E] text-white p-10 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 select-none">
              <span className="text-2xl font-black font-mono tracking-tight">
                <span className="text-orange-500">{t.brandNamePart1 || "MERKATO"}</span>{" "}
                <span className="text-white">{t.brandNamePart2 || "STORE"}</span>
              </span>
            </div>
          ...
          <div className="relative space-y-5">
            <h1 className="text-3xl xl:text-4xl font-black leading-tight font-mono uppercase tracking-tight">
              {t.welcomeBackBig}
              <span className="block text-emerald-400">{t.welcomeBackSuffix}</span>
            </h1>
        ...
        <div className="p-8 sm:p-10 lg:p-12 flex flex-col justify-center">
          <div className="max-w-sm w-full mx-auto space-y-6">
            {/* Brand accent racetrack */}
            <div className="w-16 h-1.5 rounded-full bg-gradient-to-r from-emerald-500 to-orange-500 mx-auto" />

            {/* Avatar badge */}
            <div className="flex justify-center">
              <span className="inline-flex items-center justify-center h-16 w-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-700 text-white shadow-lg shadow-emerald-600/30 ring-4 ring-emerald-50">
                <svg
                  className="h-7 w-7"
                  viewBox="0 0 24 24"
                  fill="none"
            ...
            <form className="space-y-4" onSubmit={handleAuthenticationSubmit}>
              ...
              <div>
                <label
                  htmlFor="login-email"
                  className="block text-[11px] font-black uppercase text-slate-500 tracking-wider mb-1.5"
                >
                  {t.emailAddress}
                </label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    autoComplete="email"
                    aria-invalid={errorStatusMessage ? "true" : undefined}
                    className="w-full bg-slate-50/60 border border-slate-200 text-sm rounded-xl pl-11 pr-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-400 focus:bg-white transition-all text-slate-800 placeholder-slate-400 font-medium min-h-[48px]"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    aria-invalid={errorStatusMessage ? "true" : undefined}
                    className="w-full bg-slate-50/60 border border-slate-200 text-sm rounded-xl pl-11 pr-12 py-3.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-400 focus:bg-white transition-all text-slate-800 placeholder-slate-400 font-medium min-h-[48px]"
                    required
                  />
                  <button
                    type="button"
            ...
            <footer className="text-center pt-4 space-y-2 text-[11px] font-bold border-t border-slate-100">
